...
export const opencodeCliDestination: ContextDestination = {
  id: "opencode",
  label: "OpenCode",
  aliases: ["opencode", "oc"],
  ...
  async send(context, options): Promise<SendResult> {
    ...
    if (rendered.length <= MAX_ARG_PROMPT_LENGTH) {
      messages.push(`Launching OpenCode in ${cwd} with the exported context.`);
      await execa("opencode", ["--prompt", rendered, cwd], { stdin: "inherit", stdout: "inherit", stderr: "inherit" });
    } else {
      ...
      messages.push(`Launching OpenCode in ${cwd} — paste with Cmd+V to hand off.`);
      await execa("opencode", [cwd], { stdin: "inherit", stdout: "inherit", stderr: "inherit" });
    }
    return { messages };
  }
};
